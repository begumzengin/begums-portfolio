      ...
      <main className="flex-1">
        ...
        <section id="education-experience" className="section-accent py-20 px-4 sm:px-6 lg:px-8 relative min-h-screen">
            ...
            <div className="max-w-7xl mx-auto w-full h-full">
              <div className={`transition-all duration-500 ease-in-out ${selectedSection ? 'flex' : 'block'}`}>
                {/* Sidebar */}
                <div className={`transition-all duration-500 ease-in-out transform ${selectedSection ? 'w-64 mr-8 opacity-100' : 'w-full flex flex-col items-center opacity-90'}`}>
                  <h2 className="text-3xl font-bold mb-8 text-center transition-all duration-500 ease-in-out">experience</h2>
                  <div className={`flex transition-all duration-500 ease-in-out transform ${selectedSection ? 'flex-col space-y-4' : 'space-x-4 justify-center'}`}>
                    <button
                      onClick={() => setSelectedSection('education')}
                      className={`px-6 py-3 rounded-lg transition-all duration-300 ease-in-out transform hover:scale-105 ${selectedSection === 'education' ? 'bg-accent text-foreground' : 'bg-foreground/10 hover:bg-foreground/20'}`}
                    >
                      education
                    </button>
                    <button
                      onClick={() => setSelectedSection('experience')}
                      className={`px-6 py-3 rounded-lg transition-all duration-300 ease-in-out transform hover:scale-105 ${selectedSection === 'experience' ? 'bg-accent text-foreground' : 'bg-foreground/10 hover:bg-foreground/20'}`}
                    >
                      experience
                    </button>
                  </div>
                ...
                {/* Content Area */}
                {selectedSection && (
                  <div className="flex-1 bg-foreground/5 rounded-lg p-6 content-transition-enter content-transition-enter-active">
                              ...
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <div className="relative cursor-pointer" onMouseEnter={() => {
                                  ...
                                  <div id="gallery-modal" 
                                      className="fixed inset-0 hidden items-center justify-center z-[9999] transition-opacity duration-300 opacity-0"
                                      onClick={(e) => {
                                        if (e.target === document.documentElement || e.target === e.currentTarget) {
                                          const modal = document.getElementById('gallery-modal');
                                          if (modal) {
